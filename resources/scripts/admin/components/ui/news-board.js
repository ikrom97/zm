...
function NewsBoard() {
  ...
  const handleDeleteClick = (id, title) => () => {
    window.confirm(`Вы уверены что хотите безвозвратно удалить ${title}?`) &&
      axios.delete(generatePath(ApiRoute.News['delete'], { id }))
      ...
      renderCell: (params) => (
        <Stack spacing={1} direction="row">
          <IconButton
            ...
            size="small"
            href={generatePath(AppRoute.News['edit'], { id: params.row.id })}
            color="warning"
            title="Редактировать"
          >
            <EditIcon />
  ...
  const handleDeleteSelectedButtonClick = () => {
    window.confirm(
      ...
      axios.post(ApiRoute.News['multidelete'], { ids: selection })
